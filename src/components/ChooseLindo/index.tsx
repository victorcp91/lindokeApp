...
type propTypes = {
  chooseLindo: (lindo: lindo) => void;

};
import { SafeArea, Container,MonsterContainer, NameTextInput, Start, StartText } from './styles';
 
const ChooseLindo: React.FC<propTypes> = ({chooseLindo}) => {
  const [lindoIndex, setLindoIndex] = useState<number|null>(null);
  const [lindoName, setLindoName] = useState<string>('');
  ...
  const sort = (counter: number) => {
    setTimeout(() => {
      if(counter === 7){
        finish();
      }else {
        const currentLindo = Math.floor(Math.random() * (100 - 0));
        setLindoIndex(currentLindo);
        sort(counter+1);
      }
    }, 10);
  };
  ...
  const start = () => {
    if(lindoIndex){
      chooseLindo({lindoName, lindoIndex});
    }
  }
        ...
        <Container>
          <MonsterContainer>
            {Monsters.map((Monster, index) => (
              <Monster key={index} style={{display: index === lindoIndex ? 'flex' : 'none'}} width={150}/>
            ))}
          </MonsterContainer>
          {ready && (
            <>
            <NameTextInput maxLength={8} value={lindoName} onChangeText={setLindoName} placeholder='Como se chama essa lindeza?'/>
            <Start onPress={start} disabled={!lindoName}>
              <StartText>Começar!</StartText>
            </Start>
            </>
          )}
        </Container>
